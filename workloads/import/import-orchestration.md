...
The shell implementation should be a thin task runner around the receipt contract: one command per task, JSON stdout captured directly to an immutable result artifact, structured logs on stderr, explicit gate validation, and `resume --receipt <uri>` / `status --receipt <uri>` / `retry-task --receipt <uri> --task <name> --acknowledge <reason>` commands. `resume` covers interruption; a receipt recording a failed task requires the operator acknowledgement `retry-task` carries, because only an operator can confirm subsystem state matches the receipt. Clearing a failed task must archive the failed attempt rather than remove it, and receipt sequence numbers must be forward-only so that resuming from an older receipt cannot overwrite newer evidence. It should accept the existing environment variables and/or scheduler parameters plus secret/runtime bindings; a thin import profile is optional, not required.
